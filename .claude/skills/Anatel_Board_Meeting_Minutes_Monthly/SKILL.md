...
- 脚本输出文件路径和字符数，使用Read工具读取输出文件内容进行分析。
...
- 如果因为youtube限制，无法从对应的youtube视频中获取字幕，请直接使用 .claude/skills/Anatel_Board_Meeting_Minutes_Monthly/ 文件夹中对应月份的youtube视频字幕文件，例如4月份的字幕文件为：youtube_transcript_April.txt。
...
- 报告格式参考".claude/output-styles/template-dashboard.html"格式。

- 报告文件名命名：Anatel董事会月度会议纪要分析 + _ + 会议年份 + 月份 + dashboard .html

- 报告最后不要有任何多余的内容。

- 输出报告存放到".claude/output"路径下。
